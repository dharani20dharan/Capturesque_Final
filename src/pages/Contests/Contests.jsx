...
import './Contests.css'; // You can reuse Gallery.css or rename your contests.css to match

/* ------------------
   Config
------------------- */
// Use environment variables in a real-world app, but this is fine for this context.
let API_BASE_URL = 'http://150.230.138.173:8087';

const ROOT_FOLDER = 'Contests'; // *** THIS IS THE ONLY MAJOR CHANGE NEEDED ***
const PAGE_SIZE = 24;
...
/* ------------------
   Helpers (Copied from Gallery.jsx)
------------------- */
...
export default function Contests() {
  ...
  const [sortOrder, setSortOrder] = useState('nameAsc'); // Or default sort
  const [extFilter, setExtFilter] = useState('all'); // Or default filter
  ...
  // All logic functions from Gallery.jsx are copied here, unchanged.
  ...
  const fetchRootFolders = async () => {
    ...
    try {
      ...
      setError('Failed to load contest folders.');
      setFolders([]);
    } finally {
      setLoading(false);
    }
  ...
  const goUpOneLevel = async () => {
    ...
    if (parts.length <= rootParts.length) { // Go back to root
    ...
    const parentName = parts.length > rootParts.length ? parts[parts.length - 1] : ROOT_FOLDER;
    if (parentId === normalizePathParts(ROOT_FOLDER).join('/')) {
        setSelectedFolder(null);
        setImagesAll([]);
        setSubfolders([]);
        await fetchRootFolders();
          ...
          <div className="folders" style={{ marginTop: 12 }}>
            {visibleFoldersFiltered.map(folder => (
              <div key={folder.folderId} className="folder-card" onClick={() => openFolder(folder)} role="button" tabIndex={0}>
                ...
                <h3 className="folder-name" style={{ display:'flex', alignItems:'center', justifyContent: 'center', gap:8 }}>
                  {folder.folderName}
                  {isAdmin && (
                 ...
                 <div className="folders" style={{ marginTop: 8 }}>
                     {subfolders.map(sf => (
                         <div key={sf.folderId} className="folder-card" onClick={() => openFolder(sf)} style={{minHeight: 120}}>
                           <FaFolder className="folder-icon"/>
                           <h4 className="folder-name" style={{ display:'flex', alignItems:'center', justifyContent: 'center', gap:8 }}>
                             {sf.folderName}
                             {isAdmin && (
        ...
        {isUploading && (
            <div style={{ marginTop: 12 }}>
                ...
                <div style={{ width: '100%', background: '#333', height: 8, borderRadius: 4, marginTop: 4 }}>
                    <div style={{ width: `${uploadProgress.overall}%`, height: '100%', background: 'var(--accent-primary, #007bff)', borderRadius: 4 }} />
                </div>
            </div>
        )}
        {selectedFolder && (
